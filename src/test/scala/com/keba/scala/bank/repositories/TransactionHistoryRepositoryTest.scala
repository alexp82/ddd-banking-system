...
import com.keba.scala.bank.services.BankingTestConstants
import com.keba.scala.bank.transactions.{DepositTransactionHistoryEntry, RegistrationTransactionHistoryEntry, TransactionHistoryEntry}
...
class TransactionHistoryRepositoryTest extends FunSuite with BeforeAndAfterEach {
  ...
  test("It should not be possible to insert a transaction history entry" +
    ...
    val theRetrievedEntries: List[TransactionHistoryEntry] = TransactionHistoryRepository.read(BANK_ACCOUNT_1)
  ...
  test("It should be possible to retrieve a list of transaction history" +
    ...
    val theDepositEntry = new DepositTransactionHistoryEntry(new Date(), BANK_ACCOUNT_1, theDepositMoney)
