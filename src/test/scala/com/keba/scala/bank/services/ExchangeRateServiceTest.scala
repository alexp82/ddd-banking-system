...
class ExchangeRateServiceTest extends FunSuite with BeforeAndAfterEach {
  ...
  test("If several exchange rates from currency A to currency B are registered," +
    " the last registered exchange rate should be used when calculating an exchange") {
    /* Register exchange rate from TWD to SEK. */
    ...
    theExchangedSekMoneyOption = exchangeRateService.exchange(MONEY_40_TWD, CURRENCY_SEK)
    assert(theExchangedSekMoneyOption.isDefined)
  ...
  test("It should always be possible to calculate the exchange from " +
    ...
    val theExchangedSekMoneyOption = exchangeRateService.exchange(MONEY_10_SEK, CURRENCY_SEK)
    assert(theExchangedSekMoneyOption.isDefined)
    val theExchangedSekMoney = theExchangedSekMoneyOption.get
