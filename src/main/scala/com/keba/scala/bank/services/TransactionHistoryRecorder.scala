...
trait TransactionHistoryRecorder extends BankingServiceMixinInterface {
  ...
  abstract override def balance(inBankAccountNumber: String): Money = {
     ...
     * a transaction history entry should be generated.
     */
    val theTransactionHistoryEntry = new BalanceInquiryTransactionHistoryEntry(theEntryTime, inBankAccountNumber)
    transactionHistoryService.addTransactionHistoryEntry(theTransactionHistoryEntry)
  ...
  /**
    ...
    */
  abstract override def deposit(inBankAccountNumber: String, inDepositAmount: Money): Unit = {
    /* Get transaction history entry timestamp as time of request. */
    val theEntryTime = new Date()
    /*
    ...
    val theLocalCurrencyDepositAmount = thePostDepositBalance.amount - thePreDepositBalance.amount
    /*
    ...
    if (inDepositAmount.currency == thePreDepositBalance.currency) {
      ...
      val theTransactionHistoryEntry = new DepositTransactionHistoryEntry(theEntryTime, inBankAccountNumber, inDepositAmount)
      transactionHistoryService.addTransactionHistoryEntry(theTransactionHistoryEntry)
    } else {
      /* Foreign currency was deposited. */
      val theLocalCurrencyDeposit = new Money(theLocalCurrencyDepositAmount, thePreDepositBalance.currency)
      /*
       * Calculate the exchange rate used for the deposit without
       * using any external service.
       */
      val theExchangeRate = theLocalCurrencyDepositAmount / inDepositAmount.amount
      val theTransactionHistoryEntry = new ForeignCurrencyDepositTransactionHistoryEntry(
  ...
  /**
    ...
    */
  abstract override def withdraw(inBankAccountNumber: String, inWithdrawAmount: Money): Unit = {
    /* Get transaction history entry timestamp as time of request. */
    val theEntryTime = new Date()
    /*
    ...
    val theLocalCurrencyWithdrawAmount = thePreWithdrawBalance.amount - thePostWithdrawBalance.amount
    /*
    ...
    if (inWithdrawAmount.currency == thePreWithdrawBalance.currency) {
      ...
      val theTransactionHistoryEntry = new WithdrawalTransactionHistoryEntry(theEntryTime, inBankAccountNumber, inWithdrawAmount)
      transactionHistoryService.addTransactionHistoryEntry(theTransactionHistoryEntry)
    } else {
      /* Foreign currency was withdrawn. */
      val theLocalCurrencyWithdraw = new Money(theLocalCurrencyWithdrawAmount, thePreWithdrawBalance.currency)
      /*
       * Calculate the exchange rate used for the withdrawal without
       * using any external service.
       */
      val theExchangeRate = theLocalCurrencyWithdrawAmount / inWithdrawAmount.amount
      val theTransactionHistoryEntry = new ForeignCurrencyWithdrawalTransactionHistoryEntry(
