...
trait BankingServiceExceptionTranslation extends BankingServiceMixinInterface {
  ...
  abstract override def registerBankAccount(inNewBankAccount: BankAccount): Unit = {
    try {
      ...
      case _: AssertionError =>
        throw new BankAccountAlreadyExists("Failed to register new bank account. " +
          "An account with number " + inNewBankAccount.accountNumber + " has already been registered.")
  ...
  abstract override def balance(inBankAccountNumber: String): Money = {
    try {
      ...
      /* Wrap unexpected exceptions. */
      case theException: Throwable =>
        throw new Error("Failed to retrieve balance for bank account " + inBankAccountNumber, theException)
    }
  }
  ...
  abstract override def deposit(inBankAccountNumber: String, inAmount: Money): Unit = {
    try {
      ...
      /* Wrap unexpected exceptions. */
      case theException: Throwable =>
        throw new Error("Failed to deposit money to bank account " + inBankAccountNumber, theException)
    }
  }
  ...
  abstract override def withdraw(inBankAccountNumber: String, inAmount: Money): Unit = {
    try {
      ...
      /* Wrap unexpected exceptions. */
      case theException: Throwable =>
        throw new Error("Failed to withdraw money from bank account " + inBankAccountNumber, theException)
    }
  }
  ...
  abstract override protected def exchangeMoney(inAmount: Money, inToCurrency: Currency): Option[Money] = {
    try {
      ...
      if (theExchangedMoneyOption.isEmpty) {
        throw new NoExchangeRateRegistered("No exchangerate registered for exchange from " +
          inAmount.currency.toString() + " to " + inToCurrency.toString())
      }
