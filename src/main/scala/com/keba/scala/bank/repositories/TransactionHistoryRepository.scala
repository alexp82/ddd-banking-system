...
object TransactionHistoryRepository {
  /* Constant(s): */
  /* Field(s): */
  private var transactionHistoryEntries: mutable.Map[String, mutable.ListBuffer[TransactionHistoryEntry]] = mutable.Map()
  ...
  def create(inEntry: TransactionHistoryEntry): Unit = {
    ...
    require(inEntry.bankAccountNumber != null, "transaction history entry must contain a bank account number")
  ...
  def read(inBankAccountNumber: String): List[TransactionHistoryEntry] = {
    require(inBankAccountNumber != null, "cannot retrieve transaction history for null bank account number")
    val theExistingEntries = findOrCreateEntriesForAccount(inBankAccountNumber)
  ...
  /**
    ...
    */
  private def findOrCreateEntriesForAccount(inBankAccountNumber: String): mutable.ListBuffer[TransactionHistoryEntry] = {
    transactionHistoryEntries.getOrElseUpdate(inBankAccountNumber, new mutable.ListBuffer[TransactionHistoryEntry])
  }
