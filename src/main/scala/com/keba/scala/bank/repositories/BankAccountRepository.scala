...
object BankAccountRepository {
  ...
  def create(inNewBankAccount : BankAccount) : Unit = {
     ...
     */
    assume(accounts.contains(inNewBankAccount.accountNumber) == false, "a bank account with the account number " + inNewBankAccount.accountNumber + " already exists")
  ...
  def update(inBankAccount : BankAccount) : Unit = {
     ...
     * in the repository.
     */
    assume (accounts.contains(inBankAccount.accountNumber), "cannot update bank account with account number " + inBankAccount.accountNumber + " since it does not exist in the repository")
